...
export default class Compiler {
    constructor( argv ) {
        ...
        if ( this.argv.config ) {
            ...
            this.transforms.forEach( transform => {
                this.verbose( chalk.grey( '  ' + transform ) )
            })
    ...
    compile = () => {
        ...
        fs.stat( entrypath, err => {
            ...
            this.transforms.unshift( compiler )
            this.transforms.push( ws )
            //
            // // Pipe from read, through transforms and into write
            this.transforms.reduce( ( prev, curr ) => {
                if ( !curr ) {
                    return prev
                }
            ...
            // let pipeline = rs
            // //
            // // // Pipe input to the compiler
            // pipeline = pipeline.pipe( compiler )
            // pipeline = pipeline.pipe( this.transforms[ 0 ] )
            // //
            // // // Pipe into any transforms
            // // // if ( this.transforms ) {
            // // //     this.transforms.forEach( transform => pipeline.pipe( transform ) )
            // // // }
            // //
            // // // Whack it into the output
            // pipeline = pipeline.pipe( ws )
        })
    }
