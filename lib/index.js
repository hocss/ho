...
var argv = require('yargs').usage('Usage: $0 <command> [options]').command('compile', 'Compiles Less into CSS', function (yargs) {
  var compiler = new Compiler(makeArgs(yargs).argv);
  compiler.compile();
}).command('watch', 'Starts watching files', function (yargs) {
  var watchArgv = makeArgs(yargs).option('w', {
    alias: 'watch',
